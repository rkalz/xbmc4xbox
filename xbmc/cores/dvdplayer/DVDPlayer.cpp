...
void CDVDPlayer::GetGeneralInfo(CStdString& strGeneralInfo)
{
  if (!m_bStop)
  {
    ...
    {
      int64_t queued = m_State.file_length * 1000.0f * (float) g_guiSettings.GetInt("dvdplayercache.videotime") / 100 * GetCacheLevel() / m_State.time_total;
...
float CDVDPlayer::GetCachePercentage()
{
  ...
  fPercent += 1000.0f * (float) g_guiSettings.GetInt("dvdplayercache.videotime") * GetCacheLevel() / m_State.time_total;
