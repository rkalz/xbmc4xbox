...
void CGUIDialogProfileSettings::OnSettingChanged(SettingInfo &setting)
{
  ...
  if (setting.id == 4)
  {
    if (m_bShowDetails)
    {
      ...
      if (CGUIDialogLockSettings::ShowAndGetLock(m_iLockMode,m_strLockCode,m_bLockMusic,m_bLockVideo,m_bLockPictures,m_bLockPrograms,m_bLockFiles,m_bLockSettings,m_bIsDefault?12360:20068,g_settings.GetMasterProfile().getLockMode() == LOCK_MODE_EVERYONE || m_bIsDefault))
        m_bNeedSave = true;
    }
    else
    {
