...
void CAddonMgr::FindAddons(const TYPE &type)
{
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    AddonPtr addon;
    if (!AddonFromInfoXML(type, item->GetPath(), addon))
    {
      CLog::Log(LOGDEBUG, "ADDON: Error reading %sdescription.xml, bypassing package", item->GetPath().c_str());
      continue;
    }
    ...
    item2.SetPath(URIUtils::AddFileToFolder(addon->Path(), addon->LibName()));
...
bool CAddonMgr::AddonFromInfoXML(const TYPE &reqType, const CStdString &path, AddonPtr &addon)
{
  ...
  URIUtils::AddFileToFolder(strPath, ADDON_METAFILE, strPath);
  ...
  */
  ...
  addonProps.icon = URIUtils::AddFileToFolder(path, "default.tbn");
...
CStdString CAddonMgr::GetAddonsXMLFile() const
{
  ...
  if (g_settings.GetCurrentProfile().hasAddons())
    URIUtils::AddFileToFolder(g_settings.GetProfileUserDataFolder(),"addons.xml",folder);
  else
    URIUtils::AddFileToFolder(g_settings.GetUserDataFolder(),"addons.xml",folder);
