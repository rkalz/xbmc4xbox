...
{
	for(int i=0; i<g_settings.m_iBars; i++)
		m_pCubes[i].CleanUp();
...
bool CBoxLine::Init(LPDIRECT3DDEVICE8 pD3DDevice, float fLineZ)
{
	...
	m_fBaseHeight = 0.1f;
	m_fBarWidth = 24.0f / g_settings.m_iBars;
	m_fBaseZ = fLineZ;

	m_pCubes = (CD3DCube *)malloc(sizeof(CD3DCube) * g_settings.m_iBars);
	if(!m_pCubes)
		return false;
	...
	for(int i=0; i<g_settings.m_iBars; i++)
	{
		...
		m_pCubes[i].Init(m_pd3dDevice, nextLeft, -10.0f, fLineZ, m_fBaseHeight, m_fBarWidth, g_settings.m_fBarDepth);
	}
...
{
	if(m_fBaseZ + g_settings.m_fBarDepth > fCamPos)
		return true;
	else
		return false;
}